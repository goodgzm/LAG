...
from ..reward_functions import AltitudeReward, HeadingReward, PostureReward, EventDrivenReward
...
class HumanSingleCombatTask(BaseTask):
    ...
    def __init__(self, config):
        ...
        self.use_baseline = getattr(self.config, 'use_baseline', False) #gzm新
        self.use_artillery = getattr(self.config, 'use_artillery', False) #gzm新
        if self.use_baseline:
            self.baseline_agent = self.load_agent(self.config.baseline_type) #gzm新
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        """
        if self.use_baseline and agent_id in env.enm_ids: #gzm新
...
class PursueAgent(BaselineAgent): #gzm新
    def __init__(self) -> None:
        super().__init__()
...
class ManeuverAgent(BaselineAgent): #gzm新
...
class DodgeMissileAgent: #gzm新
    def __init__(self) -> None:
        self.model_path = get_root_dir() + '/model/actor_latest.pt'
