...
def main(args):
    ...
    agent = HumanAgent(env, is_shooter = True)  # 不需要显式传递 task，env 中已包含
   ...
   # 重置环境，获取初始观察状态
    ...
    while not done:
        try:
            ...
            # 可以加入适当的延时控制，避免过快执行
            time.sleep(0.2)  # 设置每一步之间的间隔时间（单位：秒），根据需求调整
                
                    # 检查是否命中并触发爆炸延时显示
            if done:
                explosion_duration = 5.0  # 显示爆炸画面2秒
                steps = int(explosion_duration / 0.2)
                for _ in range(steps):
                    ...
                    try:
                        ...
                        logging.error(f"Tacview explosion rendering error: {e}")
                        logging.error("".join(traceback.format_exc()))
                    timestamp += 0.2
                    time.sleep(0.2)
